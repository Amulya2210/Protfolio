...
export default function About() {
	...
	return (
		<div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-indigo-900 to-blue-900 text-white relative overflow-hidden px-2 py-10">
			...
			<div className="z-10 flex flex-col items-center w-full max-w-xl relative">
				<div
					className="absolute left-1/2 -translate-x-1/2 h-full w-1 bg-gradient-to-b from-yellow-400/40 via-blue-400/30 to-purple-400/30"
					style={{ zIndex: 0 }}
				/>
				{timelineData.map((item, idx) => (
					<div
						key={item.year}
						className="relative w-full flex items-center mb-16 last:mb-0"
					>
						{/* Orbit line */}
						<div
							className="absolute left-1/2 -translate-x-1/2 w-1 h-full bg-gradient-to-b from-yellow-400/40 via-blue-400/30 to-purple-400/30"
							style={{ zIndex: 0 }}
						/>
						{/* Planet (timeline block) */}
						<div className={`mx-auto flex flex-col items-center w-full z-10`}>
							<button
								className={`group flex flex-col items-center focus:outline-none transition-transform duration-300 ${
									openIndex === idx ? 'scale-110' : 'hover:scale-105'
								}`}
								onClick={() =>
